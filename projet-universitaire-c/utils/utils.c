...
WorkTree* initWorkTree() {
    ...
    wt->size = MAX_WORKFILES;
    wt->n = 0;
    wt->tab = calloc(MAX_WORKFILES, sizeof(WorkFile*));
    return wt;
}
...
char* wtts(WorkTree* wt) {
    ...
    for (int i = 0; i < wt->n; i++) {
        ...
        str = realloc(str, strlen(str) + strlen(wf_str) + 2);
        strcat(str, wf_str);
        strcat(str, "\n");
        free(wf_str);
    }
...
char* saveWorkTree(WorkTree* wt, char* path) {
    for (int i = 0; i < wt->count; i++) {
        ...
        if (isFile(wf->name)) {
            char* hash = blobFile(wf->name);
            free(wf->hash);
            wf->hash = hash;
            wf->mode = getChmod(wf->name);
        } else {
            WorkTree* newWT = listdir(wf->name);
            char* hash = saveWorkTree(newWT, wf->name);
            free(wf->hash);
            wf->hash = hash;
            wf->mode = getChmod(wf->name);
            freeWorkTree(newWT);
...
void restoreWorkTree(WorkTree* wt, char* path) {
    for (int i = 0; i < wt->n; i++) {
        ...
        char* full_path = joinPath(path, wf->name);
        if (isSnapshot(wf->hash)) {
            char* dir_path = getPath(wf->hash);
            WorkTree* newWT = ftwt(dir_path);
            makePath(full_path);
            restoreWorkTree(newWT, full_path);
            freeWorkTree(newWT);
            free(dir_path);
        } else {
            char* file_path = getPath(wf->hash);
            copyFile(file_path, full_path);
            chmod(full_path, wf->mode);
            free(file_path);
        }
        free(full_path);
    }
